...
public class LearningWithGA2 implements LearningAgent{
...
//	private EvaluationInfo localEvaluationInfo;
	...
	public LearningWithGA2(String args){

		/* 評価値を初期化 */
		fmax = 0;

		/* 個体数分だけAgentを作成 */
		agents = new GAAgent2[popsize];
		for (int i=0; i<agents.length; i++){
			agents[i] = new GAAgent2();
		}
	...
	public void learn(){
		...
		for(int generation=0 ; generation<100 ; generation++){
			...
			compFit();
			GAAgent2 nextagents[] = new GAAgent2[popsize];
			for (int i=bestnum; i<popsize; i++){
				nextagents[i] = new GAAgent2();
			}
	...
	private void compFit() {
		...
		for(int i=0 ; i<popsize ; i++){
			...
			agents[i].setDistance(evaluation);
...
//			writeFile();			//bestAgentのxmlを出力
	...
	/* 交叉 */
	private void cross(GAAgent2[] nextagents, int[] parentsGene, int i) {
